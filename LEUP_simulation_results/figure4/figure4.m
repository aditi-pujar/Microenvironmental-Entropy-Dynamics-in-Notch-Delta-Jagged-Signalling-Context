% Figure 4 - Lateral inhibition and induction in Notch Delta systems
...
cell_unimodal = {'../Datasets/LEUP/datasets/square', ...
                '/r=1/minuscircle_beta-100_r1_t%d.mat', ...
                '../Datasets/NDJ/', ...
...
figure 
   ...
   yticks([])
   xticks([]); 
   print(sprintf('fig4A%d.eps',i),'-depsc')
...
%     yticks([])
%     xticks([]); 
%     print(sprintf('fig4C%d.eps',i),'-depsc')
...
%     xlim([2 51])
    ...
    set(gca,'FontSize',18);
    title('(ii) $\langle \sigma^2\rangle_L$ Time Evolution','interpreter','latex','FontSize',22); % title('(iii) $\langle\sigma^2\rangle_L v/s t$','interpreter','latex','FontSize',20);
    
    if beta(i)<0
        xlim([2 51]); 
        legend(strcat('$NDJ:(D_0,J_0) = $',sprintf('(%d, %d)', D0, J0)),'Location','southeast','interpreter','latex');
%          
...
%            'Location','southeast','interpreter','latex');
        ...
        xlim([1 51]);
        legend(strcat('$NDJ:(D_0,J_0) = $',sprintf('(%d, %d)', D0, J0)),'Location','northeast','interpreter','latex');
%       
%         legend(strcat('$NDJ:(D_0,J_0) = $',sprintf('(%d, %d)', D0, J0)), ...
%            strcat('$LEUP:(r,\beta) = $',sprintf('(1, %d)', beta(i))), ...
...
%            strcat('$LEUP:(r,\beta) = $',sprintf('(1, %d)', beta(i))), ...
%            'Location','southeast','interpreter','latex');
    print(sprintf('fig4B%d.eps',i),'-depsc')
save(sprintf('fig4_%d.mat',i));
i = i+1;
